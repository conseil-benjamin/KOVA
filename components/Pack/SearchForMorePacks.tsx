...
const SearchForMorePacks = ({ selectedPack, setSelectedPack, packs, language }: { selectedPack: string; setSelectedPack: (pack: string) => void; packs: any; language: string }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 overflow-y-auto">
                {packs.filter(pack => pack.name[language].toLowerCase().includes(search.toLowerCase())).map(pack => (
                    <div
                        key={pack.id}
                    ...
                    >
                        ...
                        {selectedPack === pack.id && <div className={`absolute inset-0 bg-gradient-to-br ${pack.color} opacity-10`}></div>}
