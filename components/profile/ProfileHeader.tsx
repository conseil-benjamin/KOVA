...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ user, isOwner }) => {
    ...
    return (
        <div className="relative w-full rounded-2xl md:rounded-3xl p-5 md:p-8 mb-6 md:mb-8 overflow-hidden bg-slate-800/50 border border-white/10 backdrop-blur-xl">
            {/* Background Decor */}
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
                ...
                <div className="relative flex-shrink-0">
                    <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full p-1 bg-gradient-to-br from-indigo-500 to-purple-600 shadow-2xl shadow-indigo-500/20">
                ...
                <div className="flex-1 min-w-0 w-full text-center md:text-left">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
                        <div className="min-w-0">
                            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-white tracking-tight flex flex-wrap items-center gap-2 md:gap-3 justify-center md:justify-start">
                                <span className="truncate max-w-full">{user.username}</span>
                                {user.title && (
                                    <span className="text-xs md:text-sm font-semibold tracking-wide text-indigo-300 bg-indigo-500/20 px-3 py-1 rounded-full border border-indigo-500/20">
                                        {user.title}
                                    </span>
                                )}
                            </h1>
                            <p className="text-slate-400 mt-2 text-xs md:text-sm font-medium">Membre depuis le {/* Format date nicely */} {new Date(user.createdAt).toLocaleDateString()}</p>
                        </div>
                    ...
                    {/* XP Progress Bar */}
                    <div className="mt-5 md:mt-6 bg-slate-900/50 rounded-2xl p-4 border border-white/5">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-1 mb-2">
                            <span className="text-xs md:text-sm font-bold text-slate-300 uppercase tracking-wider">Progression globale</span>
                            <span className="text-xs md:text-sm font-mono text-indigo-400">
